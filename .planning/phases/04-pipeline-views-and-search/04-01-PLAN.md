...
must_haves:
  truths:
    ...
    - "User can search pipeline by firm name, contact name/email, strategy notes, and key objections with instant results"
...
NOTE — Activity description search deferred: User decision (CONTEXT.md) includes "Activity descriptions" in search scope. However, implementing real-time activity search (<500ms per keystroke per success criteria) across all investors would require either pre-loading all activities for all investors (memory intensive for 100+ investors with 50+ activities each) or a server-side search endpoint with Postgres full-text search (adds complexity beyond client-side filtering). For Phase 4, search is limited to investor table fields (firm_name, contact name/email, current_strategy_notes, key_objection_risk). Activity description search can be added in Phase 6 when full-text search infrastructure is available.
</objective>
...
9. Filter persistence across tabs: apply a search query and stage filter in Table tab, switch to Board tab, switch back to Table tab — search and filter values remain active and results still match
