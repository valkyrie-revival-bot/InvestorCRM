...
must_haves:
  truths:
    ...
    - "System prompt instructs AI to automatically invoke getInvestorDetail when firm names are mentioned in conversation (AI-05)"
  artifacts:
...
- **Automatic context surfacing (AI-05):** Instruct the agent that whenever a user mentions a firm name or investor name in conversation (even in passing, e.g. "what about Blackstone?" or "I just had a call with Sequoia"), it MUST automatically invoke `getInvestorDetail` to fetch that investor's current data BEFORE responding. The agent should never answer questions about a specific investor from memory or fabricated data — always fetch fresh context first. Include example patterns in the prompt:
  - "If the user mentions a firm name, call getInvestorDetail immediately to surface their current pipeline status, contacts, and recent activity."
  - "If the user's message references multiple firms, call getInvestorDetail for each one."
  - "Proactively surface relevant context: when discussing strategy or next steps for an investor, automatically fetch their latest data rather than waiting to be asked."
  </action>
  <verify>
