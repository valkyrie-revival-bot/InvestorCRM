...
<task type="auto">
  <name>Task 1: Install Skeleton component and create core loading screens (dashboard, investors, investor detail)</name>
  <files>components/ui/skeleton.tsx, app/(dashboard)/loading.tsx, app/(dashboard)/investors/loading.tsx, app/(dashboard)/investors/[id]/loading.tsx</files>
  <action>
First, verify if `components/ui/skeleton.tsx` exists:
```bash
test -f components/ui/skeleton.tsx && echo "Skeleton exists" || echo "ERROR: Skeleton missing — installing"
```
...
After installation (or confirming existence), verify the file is present:
```bash
test -f components/ui/skeleton.tsx || echo "ERROR: Skeleton component still missing after install"
```

Then create loading.tsx files for the 3 core routes. Each skeleton MUST match the actual layout of its corresponding page.
...
```bash
test -f components/ui/skeleton.tsx || echo "ERROR: Skeleton missing"
```

Run `npm run build` to confirm all loading.tsx files compile.
Verify core route directories have a loading.tsx:
```bash
ls app/(dashboard)/loading.tsx app/(dashboard)/investors/loading.tsx app/(dashboard)/investors/\[id\]/loading.tsx
```
  </verify>
  <done>Skeleton component installed/verified. Dashboard, investors list, and investor detail routes have loading.tsx skeleton screens matching their content layout. Build succeeds.</done>
</task>

<task type="auto">
  <name>Task 2: Create remaining route loading screens (linkedin import, settings, audit logs)</name>
  <files>app/(dashboard)/linkedin/import/loading.tsx, app/(dashboard)/settings/users/loading.tsx, app/(dashboard)/audit-logs/loading.tsx</files>
  <action>
Create loading.tsx files for the 3 remaining dashboard routes. Each skeleton MUST match the actual layout of its corresponding page.

**1. app/(dashboard)/linkedin/import/loading.tsx** — LinkedIn import skeleton:
```tsx
import { Skeleton } from "@/components/ui/skeleton";
import { Card, CardContent, CardHeader } from "@/components/ui/card";
...
**2. app/(dashboard)/settings/users/loading.tsx** — Users page skeleton:
```tsx
import { Skeleton } from "@/components/ui/skeleton";
...
**3. app/(dashboard)/audit-logs/loading.tsx** — Audit logs skeleton:
```tsx
import { Skeleton } from "@/components/ui/skeleton";
...
WHY: Completing the remaining routes ensures every dashboard page has skeleton feedback during loading transitions.
  </action>
  <verify>
Run `npm run build` to confirm all loading.tsx files compile.
Verify remaining route directories have a loading.tsx:
```bash
ls app/(dashboard)/linkedin/import/loading.tsx app/(dashboard)/settings/users/loading.tsx app/(dashboard)/audit-logs/loading.tsx
```
  </verify>
  <done>All 6 dashboard routes now have loading.tsx skeleton screens. Build succeeds.</done>
</task>
