...
At the top of the server component function, call getInvestors with error handling:
```tsx
...
Then compute these metrics from the `investors` array:
...
Note: Plan 02 depends on Plan 01 which also modifies dashboard-chat-wrapper.tsx. Plan 01 handles brand styling; Plan 02 handles navigation behavior. Plan 02 runs after Plan 01 completes, so read the file fresh to incorporate Plan 01's brand changes before adding navigation behavior.
  </action>
  <verify>
